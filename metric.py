...
for i in range(50):
    G = nx.read_gml('networks/Reco_group/reco' + str(i) + '.gml')
    Gt, Gp = sample(G, 0.9)
    print (i, len(Gt), len(Gp), len(Gt.edges()), len(Gp.edges()))

    pickle.dump(Gt, open('networks/Reco_group/reco_train' + str(i) + '.gml', 'wb'))
    pickle.dump(Gp, open('networks/Reco_group/reco_test' + str(i) + '.gml', 'wb'))
